...
**Local use only.** The application runs end to end on a developer machine. It is
not deployed to a public URL, and it should not be until a provider whose terms
permit public display is integrated and the conditions in `PROVIDERS.md`
section 6 are confirmed in writing. The provider seam and the change it needs
are described there.
